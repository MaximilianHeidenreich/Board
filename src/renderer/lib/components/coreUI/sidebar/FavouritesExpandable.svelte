...
<div class="px-8 w-max" style="min-width: 100%;"
    ...
    {#if open}
    <ul>
        {#each $settingsStore.favourites as boardID}
            <FolderBoardButton
                boardID={boardID}
                title={getTitle(boardID)}
                />
        {/each}
    </ul>
    {/if}
